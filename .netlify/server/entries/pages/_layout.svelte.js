...
function Navbar($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
      ...
      if (item.children) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<div class="relative"><button${attr_class(`flex items-center space-x-1 px-4 py-2 rounded-md text-sm font-bold transition-colors ${stringify("text-red-500")}`)}${attr("aria-expanded", servicesDropdownOpen)}><span>${escape_html(item.label)}</span> <svg${attr_class(`w-4 h-4 transition-transform ${stringify("")}`)} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button> `);
        {
          $$renderer2.push("<!--[-1-->");
        }
        $$renderer2.push(`<!--]--></div>`);
        ...
        $$renderer2.push("<!--[-1-->");
        $$renderer2.push(`<a${attr("href", item.href)}${attr_class(`px-4 py-2 rounded-md text-sm font-bold transition-colors ${stringify(store_get($$store_subs ??= {}, "$page", page).url.pathname === item.href ? "text-red-400" : "text-red-500 hover:text-red-400 hover:bg-white/[0.05]")}`)}>${escape_html(item.label)}</a>`);
      }
      $$renderer2.push(`<!--]-->`);
    }
