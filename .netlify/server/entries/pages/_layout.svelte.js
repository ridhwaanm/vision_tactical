...
function Navbar($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<header${attr_class(`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${stringify("bg-transparent")}`)}><div class="container mx-auto px-4"><div class="flex items-center justify-between h-20"><a href="/" class="flex items-center space-x-2 group"><img src="/images/vt.png" alt="Vision Tactical" class="h-10 w-auto transition-opacity group-hover:opacity-80"/></a> <nav class="hidden lg:flex items-center space-x-1"><!--[-->`);
    const each_array = ensure_array_like(nav);
    for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
      let item = each_array[$$index_1];
      if (item.children) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<div class="relative"><button${attr_class(`flex items-center space-x-1 px-4 py-2 rounded-md text-sm font-medium transition-colors ${stringify("text-zinc-400")}`)}${attr("aria-expanded", servicesDropdownOpen)}><span>${escape_html(item.label)}</span> <svg${attr_class(`w-4 h-4 transition-transform ${stringify("")}`)} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button> `);
        {
          $$renderer2.push("<!--[-1-->");
        }
        $$renderer2.push(`<!--]--></div>`);
        ...
        $$renderer2.push("<!--[-1-->");
        $$renderer2.push(`<a${attr("href", item.href)}${attr_class(`px-4 py-2 rounded-md text-sm font-medium transition-colors ${stringify(store_get($$store_subs ??= {}, "$page", page).url.pathname === item.href ? "text-white" : "text-zinc-400 hover:text-white hover:bg-white/[0.05]")}`)}>${escape_html(item.label)}</a>`);
      }
      $$renderer2.push(`<!--]-->`);
    }
...
function Footer($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<footer class="bg-[#09090B] border-t border-zinc-800"><div class="container mx-auto px-4 py-16"><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12"><div><img src="/images/vt.png" alt="Vision Tactical" class="h-10 w-auto mb-6"/> <p class="text-zinc-400 text-sm leading-relaxed mb-6">Vision Tactical is Johannesburg's trusted private security company, providing comprehensive
          security solutions including armed response, manned guarding, drone surveillance, and
          anti-kidnapping operations.</p> <div class="flex space-x-4"><!--[-->`);
